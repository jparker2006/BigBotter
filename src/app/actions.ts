...
export async function generateReplayTape(seed: number, useHaiku = false): Promise<SeasonTape> {
  ...
  if (useHaiku) {
    ...
    decider = new HaikuDecider();
  } else {
    decider = new RandomDecider(rng);
  }

  return runSeasonWithDecider(safeSeed, state0, decider, rng);
}
